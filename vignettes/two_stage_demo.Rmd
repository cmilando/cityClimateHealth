---
title: "Heat-Health Associations across multiple towns using `cityClimateHealth`"
output:
  rmarkdown::html_vignette:
    fig.retina: 2
    dev: svg
vignette: >
  %\VignetteIndexEntry{Heat-Health Associations across multiple towns using `cityClimateHealth`}
...
First create the inputs, using the same `exposure_columns` and `outcome_columns` as before. Again, remember that this data is **simulated**. 
```{r multi_town_setup}
library(data.table)
...
```{r grp, fig.height=3, fig.width=5}
ma_model$`_`$grp_plt
```

You can also make a forest plot at a specific exposure value
```{r multi_plot_multiple1, fig.height=3, fig.width=5}
...
```{r multi_plot_multiple2, fig.height=5, fig.width=8}
data("ma_towns")
ma_towns
head(ma_towns)
...
There may also be situations where you want to overwrite the strata to be a 
sub-strata level.
