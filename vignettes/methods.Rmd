...
For any questions that you don't see here, we have an additional [`FAQ`](faq.html)
...
Note that only the exposure lag is defined in [`make_exposure_matrix()`](), `argvar`, `arglag` and a new (potentially shorter) `maxlag` are defined in the model functions (e.g., [`condPois_1stage()`](), [`condPois_2stage()`](), ...)

Note also that the `dlnm` outputs may be **highly sensitive** to the choices of `arglag`, `argvar`, and `maxlag` -- we have some additional code and thoughts for this in [`dlnm_sensitivity`](dlnm_sensitivity.html)

# Outcome

Any time-series of outcomes will work. Gaps and NA values in the outcome time-series are handled by [`make_outcome_tbl()`]().
...
You can use a variety of timings:
* daily
* weekly
* monthly

See the vignette [`non_daily_data`]() for examples.
...
See the vignette [`one_stage_demo`](one_stage_demo.html).
...
See the vignette [`two_stage_demo`](two_stage_demo.html).

## Spatial Bayesian methods

[...]

See the vignette [`bayesian_demo`](bayesian_demo.html).
...
See the vignette [`attributable_number`](attributable_number.html).

# Validation

We have confirmed that this code gives identical results to test scripts using external data, see [`comparison`](comparison.html)
